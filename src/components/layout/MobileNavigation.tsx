...
import { LayoutDashboard, BookOpen, Mic2, Sparkles, BarChart3, History } from 'lucide-react'

const mobileNavItems = [
  { href: '/', label: 'Beranda', icon: LayoutDashboard },
  { href: '/latihan', label: 'Latihan', icon: BookOpen },
  { href: '/setor', label: 'Setor', icon: Mic2 },
  { href: '/kartu', label: 'Kartu', icon: Sparkles },
        ...
        {mobileNavItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center justify-center py-1 px-0.5 min-w-0 flex-1 transition-all rounded-lg"
              style={{
